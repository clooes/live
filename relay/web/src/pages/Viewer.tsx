...
export function Viewer() {
  ...
  // needTap = 自动播放被浏览器拦截（iOS 低电量模式等）：此时不盖状态浮层，
  // 露出浏览器原生控件让用户点原生播放键（手势）恢复
  const { status, live, needTap } = useWhep(videoRef, room, webrtcPort);
  ...
  return (
    <div className="viewer">
      <div className="player-wrap" ref={wrapRef}>
        ...
        />
        {/* needTap 时不盖浮层：让位给浏览器原生控件的播放键 */}
        {!live && !needTap && <div className="overlay">{status}</div>}
      ...
      <div className="bar">
        ...
        <span className="room">
          <span className="sync" title="配置实时同步">
            {synced ? " · 已同步" : " · 离线"}
          </span>
        </span>
...
function RecordPanel() {
  ...
  useEffect(() => {
    if (
      stoppingId &&
      !records.some((r) => r.id === stoppingId && r.status === "recording")
    ) {
      setStoppingId(null);
    }
  }, [records, stoppingId]);
  ...
  const active =
    recording && recording.id !== stoppingId ? recording : undefined;
  ...
  async function onStop(id: string) {
    setErr("");
    setStoppingId(id); // 立即隐藏停止按钮、显示收尾中
    try {
      await recordStop(id);
    } catch (e) {
      setErr(String(e));
  ...
  return (
    <div className="recbar-wrap">
      <div className="recbar">
        ...
        {active ? (
          <button onClick={() => onStop(active.id)}>
            ■ 停止录制（
            {Math.max(0, Math.floor((now - active.started_at_ms) / 1000))}s ·{" "}
            {active.quality}）
          </button>
        ) : finalizing ? (
          ...
          <>
            ...
            <button
              className="primary"
              onClick={onStart}
              disabled={!live || !quality}
              title={live ? "" : "无直播流，开播后可录"}
            >
              ● 开始录制
            </button>
      ...
      {records.length === 0 && (
        <p className="muted">暂无录制。选清晰度后点「开始录制」。</p>
      )}
      {records.length > 0 && (
          ...
          <tbody>
            {records.map((r) => (
              <tr key={r.id}>
                ...
                <td>
                  {r.status === "recording" && (
                    <span className="rec-live">● 录制中</span>
                  )}
                  {r.status === "cutting" && (
                    <span className="muted">裁剪中…</span>
                  )}
                  {r.status === "done" && (
                    <span className="ok">完成 {r.size}</span>
                  )}
                  {r.status === "error" && (
                    <span className="rec-err" title={r.error || ""}>
                      失败
                    </span>
                  )}
                </td>
                <td>
                  {r.status === "done" && r.file && (
                    <a href={clipUrl(r.file)} download>
                      下载
                    </a>
                  )}
                </td>
