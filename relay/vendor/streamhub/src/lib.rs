...
impl StreamDataTransceiver {
        ...
        if let Some(val) = data {
            match val {
                ...
                FrameData::Audio { timestamp, data } => {
                    Self::fanout_frame(frame_senders, FrameData::Audio { timestamp, data }).await;
                }
                FrameData::Video { timestamp, data } => {
                    Self::fanout_frame(frame_senders, FrameData::Video { timestamp, data }).await;
                }
                FrameData::MediaInfo {
    ...
    /// 把一帧分发给所有订阅者；send 失败（接收端已 drop）说明该订阅者已断开，
    /// **就地把它的 sender 从 map 剔除**，避免每帧都对死通道重试 + 刷屏
    /// "Transmiter send error"（本地补丁：上游只打日志、从不清理）。
        ...
        if let Some(val) = data {
            match val {
                PacketData::Audio { timestamp, data } => {
                    Self::fanout_packet(packet_senders, PacketData::Audio { timestamp, data }).await;
                }
                PacketData::Video { timestamp, data } => {
                    Self::fanout_packet(packet_senders, PacketData::Video { timestamp, data }).await;
                }
            }
        }
    }

    /// 同 `fanout_frame`，用于 packet（WHEP 观看端走的是 RTP packet 订阅）：
    /// send 失败即剔除该 packet_sender，消除观看端断开后残留死通道的持续刷屏。
