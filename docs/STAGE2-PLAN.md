...
### 批次 6 · 录制补音频 + 下载有声/无声（R10 · 高优先，待排期）
- [ ] R10 录制补音频（record 接 FrameData::Audio + ADTS + ffmpeg 双管道混音轨 HLS + 音画同步）
- [ ] R10 下载 6 选项（original/720p/480p × 有声/无声；无声 `-an`；prepare 加 audio 参数；缓存加音频维度）
- 备注：先 mac/Linux 最小可用（3.5~4.5 天），Windows 命名管道兼容后续单列；优先级高于 R9 水印
...
- **R9 × R10 组合**：下载产物维度变成 清晰度 × 有声/无声 ×（水印 on/off）→ 缓存 key 必须三维齐全，否则串档；UI 别让选项爆炸到难用（水印可做成全局配置而非逐次选）。
